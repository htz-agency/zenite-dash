    ...
    <div className="flex-1 flex flex-col max-w-[1400px] mx-auto px-2 py-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
        <DashChartCard title="Distribuição de Valor" subtitle="Pipeline por estágio" icon={<CurrencyDollar size={18} weight="duotone" />} delay={0.2}>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={mappedPipeline} cx="50%" cy="50%" innerRadius={60} outerRadius={100} dataKey="Valor" nameKey="Estágio" animationDuration={1200}>
                  {mappedPipeline.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} stroke="white" strokeWidth={3} />)}
                </Pie>
        ...
        <DashChartCard title="Oportunidades por Estágio" subtitle="Contagem de deals" icon={<FunnelSimple size={18} weight="duotone" />} delay={0.25}>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={mappedPipeline} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#EEF1F6" vertical={false} />
                <XAxis dataKey="Estágio" tick={{ fontSize: 10, fill: "#4E6987", fontWeight: 500 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: "#98989d" }} axisLine={false} tickLine={false} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="Quantidade" name="Oportunidades" radius={[8, 8, 0, 0]} barSize={36} animationDuration={1000}>
                  {mappedPipeline.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
