...
export function DashVendas() {
  ...
  const mappedForecast = useMemo(() => forecastData.map(d => ({ Mês: d.month, Receita: d.receita, Meta: d.meta, Previsão: d.previsao })), [forecastData]);
  const mappedVelocity = useMemo(() => velocityData.map(d => ({ Mês: d.month, Dias: d.dias, Deals: d.deals })), [velocityData]);
  const mappedOwnerRevenue = useMemo(() => ownerRevenue.map(d => ({ Vendedor: d.owner, Ganho: d.ganho, Pipeline: d.pipeline })), [ownerRevenue]);
