...
export function DashOverview() {
  ...
  const mappedPipeline = useMemo(() => pipelineByStage.map(d => ({ Estágio: d.stage, Valor: d.value, Quantidade: d.count })), [pipelineByStage]);
  const mappedLeadsBySource = useMemo(() => leadsBySource.map(d => ({ Origem: d.source, Leads: d.count, Valor: d.value })), [leadsBySource]);
  const mappedMonthlyRevenue = useMemo(() => monthlyRevenue.map(d => ({ Mês: d.month, Receita: d.receita, Leads: d.leads, "Conversão %": d.conversao })), [monthlyRevenue]);
  const mappedWeeklyActivities = useMemo(() => weeklyActivities.map(d => ({ Dia: d.day, Tarefas: d.tarefas, Compromissos: d.compromissos, Ligações: d.ligacoes, Emails: d.emails })), [weeklyActivities]);
  ...
  return (
    <div className="flex-1 flex flex-col max-w-[1400px] mx-auto px-2 py-4">
      {/* Header */}
      <motion.div
        ...
        >
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={mappedMonthlyRevenue} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <defs key="ov-defs">
                ...
                <CartesianGrid key="grid" strokeDasharray="3 3" stroke="#EEF1F6" vertical={false} />
                <XAxis key="xaxis" dataKey="Mês" tick={{ fontSize: 11, fill: "#4E6987", fontWeight: 500 }} axisLine={false} tickLine={false} />
                <YAxis key="yaxis" tick={{ fontSize: 10, fill: "#98989d", fontWeight: 500 }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v / 1000}k`} />
                <Tooltip key="tooltip" content={<CustomTooltip />} />
                <Area key="area-receita" type="monotone" dataKey="Receita" name="Receita" fill="url(#revenueGrad)" stroke="#0483AB" strokeWidth={2.5} dot={false} animationDuration={1200} />
                <Bar key="bar-leads" dataKey="Leads" name="Leads" fill="#07ABDE" radius={[4, 4, 0, 0]} barSize={20} opacity={0.7} animationDuration={1000} />
                <Line key="line-conversao" type="monotone" dataKey="Conversão %" name="Conversão %" stroke="#3CCEA7" strokeWidth={2} dot={{ r: 3, fill: "#3CCEA7" }} animationDuration={1400} yAxisId={0} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </DashChartCard>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="Valor"
                  nameKey="Estágio"
                  onClick={(data) => openStageDrill(data["Estágio"])}
                  onMouseEnter={(_, i) => setHoveredStage(mappedPipeline[i]["Estágio"])}
                  onMouseLeave={() => setHoveredStage(null)}
                  animationBegin={200}
                  animationDuration={1200}
                  style={{ cursor: "pointer" }}
                >
                  {mappedPipeline.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={COLORS[i]}
                      opacity={hoveredStage === null || hoveredStage === entry["Estágio"] ? 1 : 0.3}
                      stroke="white"
                      strokeWidth={2}
                    />
                  ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
          ...
          <div className="h-[240px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={mappedLeadsBySource} layout="vertical" margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
                <CartesianGrid key="grid-src" strokeDasharray="3 3" stroke="#EEF1F6" horizontal={false} />
                <XAxis key="xaxis-src" type="number" tick={{ fontSize: 10, fill: "#98989d" }} axisLine={false} tickLine={false} />
                <YAxis key="yaxis-src" type="category" dataKey="Origem" tick={{ fontSize: 11, fill: "#4E6987", fontWeight: 500 }} axisLine={false} tickLine={false} width={70} />
                <Tooltip key="tooltip-src" content={<CustomTooltip />} />
                <Bar key="bar-leads-src" dataKey="Leads" name="Leads" fill="#07ABDE" radius={[0, 6, 6, 0]} barSize={16} animationDuration={1000}
                  onClick={(data: any) => openSourceDrill(data["Origem"])}>
                  {mappedLeadsBySource.map((_, i) => (
        ...
        >
          <div className="h-[240px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={mappedWeeklyActivities} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                <CartesianGrid key="grid-wk" strokeDasharray="3 3" stroke="#EEF1F6" vertical={false} />
                <XAxis key="xaxis-wk" dataKey="Dia" tick={{ fontSize: 11, fill: "#4E6987", fontWeight: 500 }} axisLine={false} tickLine={false} />
                <YAxis key="yaxis-wk" tick={{ fontSize: 10, fill: "#98989d" }} axisLine={false} tickLine={false} />
                <Tooltip key="tooltip-wk" content={<CustomTooltip />} />
                <Bar key="bar-tarefas" dataKey="Tarefas" name="Tarefas" stackId="a" fill="#0483AB" radius={[0, 0, 0, 0]} animationDuration={800} />
                <Bar key="bar-compromissos" dataKey="Compromissos" name="Compromissos" stackId="a" fill="#3CCEA7" animationDuration={900} />
                <Bar key="bar-ligacoes" dataKey="Ligações" name="Ligações" stackId="a" fill="#EAC23D" animationDuration={1000} />
                <Bar key="bar-emails" dataKey="Emails" name="Emails" stackId="a" fill="#ED5200" radius={[4, 4, 0, 0]} animationDuration={1100} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </DashChartCard>
